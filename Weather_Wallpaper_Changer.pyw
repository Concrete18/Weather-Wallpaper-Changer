...
class Main(Helper):
    ...
    def action(self, weather_dict):
        ...
        # Checks Time of day
        if (
            sunrise_start < dt.datetime.now() < sunrise_end
            or sunset_start < dt.datetime.now() < sunset_end
        ):
            ...
            time_of_day = "Night"
        if (
            weather_description in self.weather_dic
        ):  # converts different OpenWeather data to supported types
