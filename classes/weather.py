...
class Weather(Helper):
    def __init__(
        self, api_key, temp_unit, zipcode=None, country="us", lat=None, lon=None
    ):
        """
        Requies an `api_key`.
    ...
    def create_api_url(self, location_mode):
        ...
        if location_mode == "coord":  # Sets url to use coordinates
            ...
            api_url = (
                f"{base_url}zip={self.zipcode},{self.country}&appid={self.api_key}"
            )
        else:
    ...
    def check(self, location_mode):
        ...
        response = self.request_url(api_url)
        if not response:
        ...
        if "weather" in data:
            weather = data[
                "weather"
            ]  # store the value of "weather" key in variable weather
            ...
            sunrise_end = sunrise_start + dt.timedelta(
                minutes=sunrise_length, seconds=-1
            )
            sunset_end = sunset_time + dt.timedelta(seconds=-1)
            sunset_start = (
                sunset_end
                + dt.timedelta(minutes=-sunset_length)
